...
%Calculamos el multiplicador óptimo (offline)
...
l = zeros(N,1);
l(1) = 1/P_max; 
%Esto simplemente es una inicialización
for n=1:N
    p(n) = f_waterfilling( h(n), l(n));
    c(n) = log(1 + h(n)*p(n));
    if n<N
        %Si no estamos en el último instante, actualizamos la estimación
        %del multiplicador para el próximo instante
        l(n+1) = actualiza_est_mult(l(n),h(n));
        %Esta función hay que codificarla (es muy sencilla)
    end
end
